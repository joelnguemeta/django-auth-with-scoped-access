...
- ✅ Cas de conformité write-guard/reauth/access-summary (+ SPEC §12.1.1 scripts ReAuth, fixture `"registered": true` pour le cas anchor non résolu) ; `engine.user_covers` + `engine.access_summary` ; module `reauth/` (ReAuthService + verifiers pluggables, expiry logique évaluée contre `at`) ; glue DRF complète (`ScopedModelPermission`, `ScopeObjectPermission`, `ScopeQuerySetMixin` — filtre les listes seulement, le détail passe par l'object permission 403 vs 404 —, `RequireReAuth`/`ReAuthView`/`MeAccessView`) + smoke tests. **Suite : 7 cas de conformité + 4 tests DRF, tout vert.** Piège DRF documenté : ErrorDetail coerce les booléens → `ReAuthRequired` pose `self.detail` après init.
- ✅ Migrations du package (`makemigrations` via tests.settings) ; API émettrice des événements §9 (`ScopeAssignment.objects.grant()`, `Role.grant_permissions()/revoke_permissions()` — **jamais** `permissions.add()` direct ni `objects.create()` pour un octroi, sinon pas de signal) ; tests Django §8.3 + §9 (`tests/test_lifecycle.py`) ; LICENSE **MIT** (copyright Joel Edmond Nguemeta) ; CI GitHub Actions (matrice Python 3.11/3.13 × Django 4.2/5.2, uv). **14 tests verts.**
- ✅ Décisions actées : **projet personnel** (org GitHub perso, pas PSETEAM), licence MIT. Nom définitif toujours ouvert (`django-scoped-access` = nom de travail).
- ✅ Modèles swappables (`AbstractRole`/`AbstractScopeAssignment` + `SCOPED_ACCESS_ROLE_MODEL`/`SCOPED_ACCESS_ASSIGNMENT_MODEL`, défauts injectés par `conf.ensure_swappable_defaults()` car `Meta.swappable` exige que le setting existe) ; cache par requête (`scoped_access/cache.py` : contextvar + `ScopedAccessCacheMiddleware`, mémoïse seulement les appels à horloge implicite, invalidé en place par grant/revoke/suspend/`grant_permissions` — `role_permissions_changed` vide tout le store) ; migrations régénérées ; tests cache. **18 tests verts.**
- ✅ Décision : **pas de migration de projet existant** — DME et APSR restent des cas de référence de conception uniquement, aucun guide de migration à écrire.
- ⬜ Extension du format de conformité pour §8.3/§9 (aujourd'hui tests Django only), remote GitHub + premier push (nom du repo à confirmer par le propriétaire), publication PyPI.
